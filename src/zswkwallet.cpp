...
CzSWKWallet::CzSWKWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zswk seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSWKSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSWKSeed_deprecated()) {
                    LogPrintf("%s: Updated zSWK seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zswk seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zswk wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzSWKWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSWKCount(nCountLastUsed);
    else if (!walletdb.ReadZSWKCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSWKWallet::Lock()
{
    seedMaster = 0;
}

void CzSWKWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSWKWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSWK(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzSWKWallet::LoadMintPoolFromDB()
{
...
void CzSWKWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSWKWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSWKWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zswkTracker->HasPubcoinHash(pMint.first)) {
...
bool CzSWKWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSWK(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zswkTracker which also adds to database
    pwalletMain->zswkTracker->Add(dMint, true);
...
void CzSWKWallet::SeedToZSWK(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzSWKWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSWKWallet::GenerateDeterministicZSWK(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSWKWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSWKWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
