...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZSwkStake(spend));
    } else {
        ...
        CSwkStake* swkInput = new CSwkStake();
        swkInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(swkInput);
    }
