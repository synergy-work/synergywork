...
public:
    ...
    bool WriteZSWKSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZSWKSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZSWKSeed_deprecated(uint256& seed);
    bool EraseZSWKSeed();
    bool EraseZSWKSeed_deprecated();

    bool WriteZSWKCount(const uint32_t& nCount);
    bool ReadZSWKCount(uint32_t& nCount);
