...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::SWK);
    ...
    if (!settings.contains("nAnonymizeSynergyWorkAmount"))
        settings.setValue("nAnonymizeSynergyWorkAmount", 1000);

    nAnonymizeSynergyWorkAmount = settings.value("nAnonymizeSynergyWorkAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeSynergyWorkAmount"))
        SoftSetArg("-anonymizesynergyworkamount", settings.value("nAnonymizeSynergyWorkAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in synergywork.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeSynergyWorkAmount:
            return QVariant(nAnonymizeSynergyWorkAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeSynergyWorkAmount:
            nAnonymizeSynergyWorkAmount = value.toInt();
            settings.setValue("nAnonymizeSynergyWorkAmount", nAnonymizeSynergyWorkAmount);
            emit anonymizeSynergyWorkAmountChanged(nAnonymizeSynergyWorkAmount);
            break;
        case CoinControlFeatures:
