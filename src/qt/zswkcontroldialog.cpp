...
#include "ui_zswkcontroldialog.h"
...
std::set<std::string> ZSwkControlDialog::setSelectedMints;
std::set<CMintMeta> ZSwkControlDialog::setMints;

ZSwkControlDialog::ZSwkControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZSwkControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZSwkControlDialog::~ZSwkControlDialog()
{
    delete ui;
}

void ZSwkControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZSwkControlDialog::updateList()
{
...
void ZSwkControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZSwkControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZSwk_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZSwkControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZSwkControlDialog::GetSelectedMints()
{
...
void ZSwkControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
