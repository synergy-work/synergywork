...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered synergywork address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 SWK.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal synergywork:URI (synergywork:XyZ...?message=example)
            {
...
static void NotifyzSWKReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzSWKReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzSWKReset.connect(boost::bind(NotifyzSWKReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzSWKReset.disconnect(boost::bind(NotifyzSWKReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zswkTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
